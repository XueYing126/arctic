...
class SegmLoss(nn.Module):
    ...
    def forward(self, data_dict):
        segm_mask = data_dict['segm_mask']
        segm_valid = data_dict['segm_valid']
        segm_target = segm_mask[:, 0]
...
def compute_loss(pred, gt, meta_info, args):
    ...
    # segmentation loss
    segm_target = gt["segm_mask"]
    segm_valid = gt["segm_valid"]
    segm_dict = {}
    segm_dict["segm_mask"] = segm_target
    segm_dict["segm_valid"] = segm_valid
    segm_dict["segm_logits"] = pred['segm_logits']
    segm_loss = SegmLoss()
    segm = segm_loss(segm_dict)
