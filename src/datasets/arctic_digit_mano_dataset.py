...
class ArcticDigitManoDataset(Dataset):
    ...
    def getitem(self, imgname, load_rgb=True):
        ...
        seg_path = imgname.replace("/data/arctic_data/data/cropped_images", "/data/masks").replace(".jpg", ".png")
        try:
            img_segm = Image.open(seg_path)
            img_segm = np.array(img_segm) # (1000, 1000)
            img_segm = ( np.stack((img_segm, img_segm, img_segm), axis=2) if img_segm is not None else None ) # (1000, 1000, 3)
        except:
            img_segm = None
        
        segm_valid = targets["left_valid"] or targets["right_valid"]
        segm_valid = segm_valid and img_segm is not None
        segm_valid = int(segm_valid)

        if load_rgb and segm_valid == 1:
            img_segm = data_utils.segm_processing(
                self.aug_data,
                img_segm,
                center,
                scale,
            ...
            img_segm = torch.from_numpy(img_segm).float() 
            img_segm_224 = img_segm.clone().long()
        else:
            img_segm_224 = torch.zeros(3, 224, 224).long()
        
        # downsample to target resolution    torch.Size([3, 112, 112])
        img_segm_224 = (F.interpolate(img_segm_224[None, :, :, :].float(), 112, mode="nearest").long().squeeze())

        targets['segm_valid'] = segm_valid
        targets["segm_mask"] = img_segm_224
        ######
